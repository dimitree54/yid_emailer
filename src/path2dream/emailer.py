...
class EmailService:
    ...
    def send_email(self, template: EmailTemplateBase, to_email: str) -> bool:
        subject = template.get_subject()
        html_content = template.render()
        metadata = template.get_metadata()

        success = self.provider.send(to_email, subject, html_content, metadata)
        return success
    
    def send_confirmation_email(self, purchase_info: str, recipient_email: str) -> bool:
        ...
        return self.send_email(template, recipient_email)
    
    def send_results_email(self, result_doc_link: str, recipient_email: str) -> bool:
    ...
    def send_developer_notification(self, error: str, company_name: str, sender_name: str, sender_email: str) -> bool:
        template = ErrorReportTemplate(error, company_name, sender_name, sender_email)
        return self.send_email(template, self.developer_email)
